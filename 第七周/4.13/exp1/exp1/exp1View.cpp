...
void Cexp1View::OnDraw(CDC* pDC)
{
	...
	if (!pDoc)
		return;
	// TODO: 在此处为本机数据添加绘制代码
	//s.Format(_T("%d  %d "), cr.TopLeft().x, cr.TopLeft().y);
	s.Format(_T("%d  %d "), temp.x, temp.y);
...
void Cexp1View::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (point.y >= cr.top&&point.y <= cr.bottom  && point.x >= cr.left && point.x <= cr.right)
	{
		set = true;
		temp.x = point.x;
		temp.y = point.y;
...
void Cexp1View::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (set==true)
	{
			int dx = point.x - temp.x;
			int dy = point.y - temp.y;
			cr.top += dy;
			cr.bottom += dy;
			cr.left += dx;
			cr.right += dx;
